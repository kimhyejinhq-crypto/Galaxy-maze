...
Kiến trúc: mỗi phòng (room) là 1 GameState độc lập, lưu qua RoomStore (xem
class RoomStore bên dưới - tự chọn Redis nếu có REDIS_URL, hoặc RAM nếu không).
...
class RoomStore:
    """
    Nơi lưu trạng thái các phòng chơi.

    VÌ SAO CẦN FILE NÀY (đọc nếu bạn từng gặp lỗi "không tìm thấy phòng" khi
    deploy lên Render/hosting có nhiều container hoặc có "ngủ" khi rảnh):
    Nếu chỉ lưu state trong 1 dict Python bình thường (RAM của tiến trình),
    dữ liệu đó CHỈ tồn tại trong đúng container/tiến trình đã tạo ra nó. Nếu
    hosting chạy nhiều container song song, hoặc container bị khởi động lại
    (kể cả do "ngủ" khi không ai dùng), phòng sẽ biến mất với người chơi ở
    container/tiến trình khác.

    Cách khắc phục: nếu có biến môi trường REDIS_URL (trỏ tới một Redis dùng
    chung, ví dụ Upstash miễn phí), state được lưu ở đó - mọi container/tiến
    trình đều đọc/ghi cùng một nơi, nên không còn bị mất phòng nữa. Nếu KHÔNG
    có REDIS_URL (ví dụ khi chạy thử ở máy cá nhân), tự động dùng lại dict
    trong RAM như cũ - vẫn chạy bình thường cho việc phát triển/test.
    """
    ROOM_TTL_SECONDS = 6 * 60 * 60  # phòng tự hết hạn sau 6 tiếng không hoạt động

    def __init__(self):
        self._mem: dict[str, GameState] = {}
        self._redis = None
        self._redis_error = None
        # Lỗi CUỐI CÙNG xảy ra khi đọc/ghi Redis lúc đang chạy (khác với
        # _redis_error - đó chỉ là lỗi lúc KHỞI ĐỘNG). Nếu Redis rớt kết nối
        # giữa chừng (hết hạn free-tier, timeout, rate-limit...), trước đây
        # code sẽ ÂM THẦM coi như "không tìm thấy phòng" - giờ lỗi thật được
        # ghi lại đây và hiện ra ở /api/health để không phải đoán mò nữa.
        ...
        if redis_url:
            try:
                ...
                client.ping()
                self._redis = client
                print("[RoomStore] ✅ Đã kết nối Redis thành công - phòng chơi sẽ được lưu dùng chung, an toàn qua restart/nhiều container.", flush=True)
            except Exception as e:
                # Không kết nối được Redis (sai URL, hết hạn, mạng lỗi...) ->
                # rơi về RAM để game vẫn chơi được trên 1 container, thay vì
                # crash toàn bộ server.
                self._redis = None
                self._redis_error = str(e)
                print(f"[RoomStore] ⚠️ Có REDIS_URL nhưng KHÔNG kết nối được ({e}). Đang chạy tạm bằng RAM - phòng SẼ bị mất nếu container restart/nhiều instance!", flush=True)
        else:
            print("[RoomStore] ⚠️ KHÔNG có biến môi trường REDIS_URL - đang chạy bằng RAM. Nếu đây là môi trường production (Render...), phòng chơi SẼ bị mất khi container restart hoặc chạy nhiều instance. Xem README.md để cấu hình Redis.", flush=True)
    ...
    @property
    def redis_error(self) -> Optional[str]:
    ...
    def room_count(self) -> int:
        if self._redis:
            try:
                return len(self._redis.keys("room:*"))
            except Exception:
                return -1
    ...
    def get(self, code: str) -> Optional[GameState]:
        ...
        if self._redis:
            try:
                raw = self._redis.get(f"room:{code}")
            except Exception as e:
                self._last_runtime_error = f"GET loi: {e}"
                print(f"[RoomStore] LOI khi DOC Redis cho phong {code}: {e}", flush=True)
                return self._mem.get(code)
            if not raw:
                return self._mem.get(code)
            try:
                state = GameState.from_full(json.loads(raw))
                self._mem[code] = state
                return state
            except Exception as e:
                self._last_runtime_error = f"PARSE loi: {e}"
                print(f"[RoomStore] LOI khi PARSE du lieu phong {code} tu Redis: {e}", flush=True)
                return self._mem.get(code)
        return self._mem.get(code)

    def save(self, state: GameState):
        code = state.room_code.upper()
        self._mem[code] = state
        if self._redis:
            try:
                self._redis.setex(f"room:{code}", self.ROOM_TTL_SECONDS, json.dumps(state.to_full()))
            except Exception as e:
                self._last_runtime_error = f"SAVE loi: {e}"
                print(f"[RoomStore] LOI khi GHI phong {code} vao Redis: {e}", flush=True)

    def exists(self, code: str) -> bool:
        code = (code or "").upper()
        if self._redis:
            return self._redis.exists(f"room:{code}") > 0
        return code in self._mem
...
class GameEngine:
    ...
    def buy_item(self, code: str, player_id: int, item_type: str) -> GameState:
        ...
        # ĐÃ BỎ giới hạn tối đa 2 vật phẩm/người theo yêu cầu: giờ chỉ bị giới
        # hạn bởi VÀNG bạn có và HÀNG còn trong kho - mua tới khi hết tiền.
