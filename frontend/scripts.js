...
let isSubmitting = false;      // chống spam click nút tạo/vào phòng
let peekRoomErrorCount = 0;    // đếm số lần gọi peek phòng bị lỗi 404
...
async function peekRoomForTakenChars() {
  ...
  if (peekRoomErrorCount > 3) {
    console.warn("Đã thử quá nhiều lần, tạm dừng kiểm tra.");
    return;
  }
  try {
    ...
    if (data.success) {
      ...
      peekRoomErrorCount = 0;
    } else {
      if (data.error && data.error.includes("không tìm thấy")) {
        peekRoomErrorCount++;
      }
    }
  } catch (e) {
