...
let cameraMode = "follow";    // "follow" (theo người chơi hiện tại) | "full" (toàn cảnh bản đồ)
...
function validateSetupForm() {
  if (isSubmittingSetup) return; // không cho form validation ghi đè trạng thái "đang xử lý"
...
$("#btn-submit-setup").addEventListener("click", async () => {
  if (isSubmittingSetup) return; // chặn bấm dồn dập khi đang chờ phản hồi server
  ...
  btn.innerHTML = `<span class="btn-spinner"></span> ĐANG KẾT NỐI TỚI TRẠM...`;
  ...
  try {
    ...
    // Thành công -> sắp chuyển màn hình, không cần khôi phục nút nữa.
  } catch (e) {
    errEl.textContent = e.message || "Có lỗi xảy ra. Server có thể đang khởi động lại (đặc biệt lần đầu sau một lúc không ai vào) - hãy thử lại sau vài giây.";
    // Thất bại -> khôi phục nút để người chơi có thể thử lại.
...
function tileCenter(idx) {
  const p = tilePositions && tilePositions[idx];
  if (!p) return { x: 0, y: 0 }; // phòng hờ: chưa có toạ độ (chưa vào ván đấu)
...
function updateCamera(state) {
  // BẢO VỆ QUAN TRỌNG: hàm này có thể bị gọi bởi sự kiện "resize" của trình
  // duyệt (ví dụ thanh địa chỉ trên di động tự ẩn/hiện, hoặc bàn phím ảo mở
  // ra) VÀO BẤT KỲ LÚC NÀO sau khi latestState được gán - kể cả lúc còn đang
  // ở phòng chờ, trước khi bản đồ mê cung được tạo và trước khi tilePositions
  // từng được tính lần nào. Không có bảo vệ này, tileCenter() sẽ đọc thuộc
  // tính của null và làm crash toàn bộ script - đây là lỗi thật đã xảy ra.
  if (!state || !state.game_started || !state.board || state.board.length === 0) return;
  if (!tilePositions) tilePositions = computeTilePositions(state.board);
  ...
  if (cameraMode === "full") {
    const trackW = track.offsetWidth || 2200;
    const trackH = track.offsetHeight || 1400;
    const scale = Math.min(rect.width / trackW, rect.height / trackH) * 0.94;
    const tx = (rect.width - trackW * scale) / 2;
    const ty = (rect.height - trackH * scale) / 2;
...
$("#btn-cam-follow").addEventListener("click", () => {
  cameraMode = "follow";
  $("#btn-cam-follow").classList.add("active");
  $("#btn-cam-full").classList.remove("active");
  if (latestState) updateCamera(latestState);
});
$("#btn-cam-full").addEventListener("click", () => {
  cameraMode = "full";
  $("#btn-cam-full").classList.add("active");
  $("#btn-cam-follow").classList.remove("active");
  if (latestState) updateCamera(latestState);
});
...
function renderShop(state) {
  ...
  $("#shop-my-gold").textContent = me.gold;
  const wrap = $("#shop-items");
  wrap.innerHTML = "";
  Object.entries(state.item_info).forEach(([key, info]) => {
    const stock = state.item_stock[key] ?? 0;
    const canBuy = stock > 0 && me.gold >= info.price;
    let reason = "";
    if (stock <= 0) reason = "Hết hàng trong kho";
    else if (me.gold < info.price) reason = "Không đủ vàng";
    const row = document.createElement("div");
    row.className = "shop-item";
    row.innerHTML = `
      <div>
        <div class="si-name">${info.emoji} ${info.name}</div>
        <div class="si-info">${info.desc} · Giá ${info.price} vàng · Còn ${stock}${reason ? ` · <span class="si-reason">${reason}</span>` : ""}</div>
      </div>`;
    const btn = document.createElement("button");
    btn.textContent = "MUA";
    btn.disabled = !canBuy;
...
function renderPendingModals(state) {
  // Cửa hàng: KHÔNG hiện trong lúc đang có bài Tarot đang lật (tarot-stage nằm
  // đè lên trên với z-index cao hơn, bấm vào nút Mua/Skip lúc đó sẽ bị bài
  // Tarot "nuốt mất" click mà không có phản hồi gì - đây chính là lỗi khiến
  // cửa hàng tưởng như bị đứng). Đợi bài lật xong (cardAnimating=false) rồi
  // mới hiện cửa hàng - processCardQueue() sẽ tự gọi lại renderPendingModals
  // ngay khi lật bài xong nên không bị trễ thêm ngoài thời gian xem bài.
  if (state.pending_shop_tile) {
    if (cardAnimating) { $("#modal-shop").classList.add("hidden"); }
    else { renderShop(state); }
  } else {
  ...
  if (pa.kind === "direction_choice") {
    ...
    if (cardAnimating) { $("#modal-pending").classList.add("hidden"); }
    else { $("#modal-pending").classList.remove("hidden"); }
    return;
  }
  ...
  if (!isMine) {
    ...
    else $("#modal-pending").classList.add("hidden");
    return;
  }

  // Không hiện modal chọn lựa (mục tiêu/ô/khu vực...) trong lúc bài Tarot còn
  // đang lật, tránh bị đè z-index và "nuốt" click y hệt lỗi cửa hàng ở trên.
  if (cardAnimating) { $("#modal-pending").classList.add("hidden"); return; }
...
function processCardQueue() {
  ...
  const continueBtn = $("#btn-tarot-continue");
  ...
  function finishCard() {
    ...
    continueBtn.removeEventListener("click", finishCard);
  ...
  // Lật bài sau 700ms (xem mặt sau bí ẩn trước cho kịch tính)
  setTimeout(() => {
    const cardData = ev.data && ev.data.card;
    $("#tarot-name").textContent = cardData ? cardData.name : (isTrap ? "Bài Bẫy" : "Bài Sự Kiện");
    $("#tarot-desc").textContent = cardData ? cardData.desc : "";
    card.classList.add("flipped");
    // Cho thời gian đọc rồi mới hiện nút "Đã đọc xong" (tránh bấm nhầm ngay
    // lúc bài vừa lật), và đặt hẹn giờ tự đóng RẤT DÀI (12s) chỉ để phòng
    // trường hợp người chơi quên bấm - không tự đóng sớm như trước nữa.
    setTimeout(() => continueBtn.classList.remove("hidden"), 400);
    fallbackTimer = setTimeout(finishCard, 12000);
  }, 700);

  continueBtn.addEventListener("click", finishCard);
}
